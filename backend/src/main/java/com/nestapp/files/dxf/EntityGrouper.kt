...
object EntityGrouper {
    ...
    fun groupEntities(entities: List<Entity>, tolerance: Double): List<GroupedEntity> {
        ...
        for (i in entityWithPathList.indices) {
            ...
            do {
                ...
                for (j in entityWithPathList.indices) {
                    if (i != j && !grouped[j]) {
                        if (canBeConnected(current.endPoints, entityWithPathList[j].endPoints, tolerance)) {
    ...
    private fun canBeConnected(
        ...
        val endToStartClose = abs(path1.endX - path2.startX) < tolerance && abs(path1.endY - path2.startY) < tolerance
        val startToEndClose = abs(path1.startX - path2.endX) < tolerance && abs(path1.startY - path2.endY) < tolerance
